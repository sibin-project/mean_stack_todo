...
function signInWithEmailAndPassword2(auth, email, password) {
  ...
  return signInWithCredential2(getModularInstance(auth), EmailAuthProvider2.credential(email, password)).catch((error) => __async(null, null, function* () {
    if (error.code === `auth/${"password-does-not-meet-requirements"}`) {
      void recachePasswordPolicy(auth);
    }
    throw error;
